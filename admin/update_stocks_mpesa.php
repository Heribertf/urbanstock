...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $checkQuery = "SELECT payment_status FROM investments WHERE user_id = ? AND investment_id = ? AND stock_id = ?";
        $check_stmt = mysqli_prepare($conn, $checkQuery);
        mysqli_stmt_bind_param($check_stmt, "iss", $edit_user_id, $edit_investment_id, $stock_id);
        ...
        $updateQuery = "UPDATE investments SET payment_status = ?, investment_status = 1 WHERE user_id = ? AND investment_id = ? AND stock_id = ?";
        $update_stmt = mysqli_prepare($conn, $updateQuery);
        mysqli_stmt_bind_param($update_stmt, "siii", $editSubStatus, $edit_user_id, $edit_investment_id, $stock_id);
