...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $machine_id = trim($_POST['machine_invest']);
    $user_id = trim($_POST['client_id']);
    $txn_id = trim($_POST['transaction_ref']);
    $plan_id = trim($_POST['investment_plan']);
    $capital = trim($_POST['capital_invested']);
    ...
    } elseif (!empty($user_id) && !empty($machine_id) && !empty($plan_id)) {
        ...
        if ($stmt = mysqli_prepare($conn, $prevPayment)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                if (mysqli_stmt_num_rows($stmt) > 0) {
                    ...
                    $insert = "INSERT INTO investments (user_id, investment_type, machine_id, plan_id, capital, investment_date, close_date, txn, payment_mode, payment_status) VALUES(?,?,?,?,?,?,?,?,?,?)";
                    if ($statement = mysqli_prepare($conn, $insert)) {
                        mysqli_stmt_bind_param($statement, "isiiisssss", $param_user_id, $param_type, $param_machine, $param_plan, $param_capital, $param_date, $param_close, $param_txn, $param_mode, $param_pay_status);
